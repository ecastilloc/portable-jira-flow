...
V2 trace and publish checks use `run.json.behaviorSpec.digest` to decide whether implementation, verification, and derived summaries still match the selected behavior. Changing `behavior-spec.md` makes dependent results stale until refreshed.

The v2 specify pipeline writes:
